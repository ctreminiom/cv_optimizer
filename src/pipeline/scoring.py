...
def compute_skill_alignment_matrix(job: dict, candidate: dict) -> list[dict]:
    ...
    for r in requirements[:18]:
        ...
        full = any(s and len(s) > 3 and s in rtext_low for s in skills) or any(
            t in rtext_low for t in techs
        )
        partial = False
        if not full:
...
def build_pre_submission_checklist(report: dict) -> list[str]:
    ...
    if missing:
        items.append(
            f"Inject these ATS keywords naturally: {', '.join(str(m) for m in missing[:8])}"
        )
...
def estimate_score_uplift(report: dict) -> dict:
    ...
    uplift = counts.get("high", 0) * 6 + counts.get("medium", 0) * 3 + counts.get("low", 0) * 1
...
def status_badge(report: Any) -> tuple[str, str, str]:
    ...
    red_flags = sum(
        len(as_dict(ev).get("red_flags") or []) for ev in (report.get("evaluations") or [])
    )
