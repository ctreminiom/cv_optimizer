...
_BODY = _make_style("Body", "BodyText", fontSize=10, leading=14,
                     spaceAfter=4, spaceBefore=0)
_BODY_SMALL = _make_style("BodySmall", "BodyText", fontSize=9, leading=12,
                            spaceAfter=2)
_H1 = _make_style("H1", "Heading1", fontSize=18, leading=22, spaceAfter=8,
                    spaceBefore=8, textColor=colors.HexColor("#1f2937"))
_H2 = _make_style("H2", "Heading2", fontSize=14, leading=18, spaceAfter=6,
                    spaceBefore=12, textColor=colors.HexColor("#1f2937"))
_H3 = _make_style("H3", "Heading3", fontSize=12, leading=16, spaceAfter=4,
                    spaceBefore=8, textColor=colors.HexColor("#374151"))
_H4 = _make_style("H4", "Heading4", fontSize=11, leading=14, spaceAfter=3,
                    spaceBefore=6, textColor=colors.HexColor("#4b5563"))
_QUOTE = _make_style("Quote", "BodyText", fontSize=10, leading=14,
                       leftIndent=12, rightIndent=8, spaceAfter=4,
                       textColor=colors.HexColor("#374151"),
                       borderColor=colors.HexColor("#9ca3af"),
                       borderPadding=4, borderWidth=0)
_TLDR = _make_style("TLDR", "BodyText", fontSize=10, leading=14,
                      leftIndent=12, rightIndent=8, spaceAfter=2,
                      spaceBefore=2,
                      textColor=colors.HexColor("#1e3a8a"))
_LIST_ITEM = _make_style("ListItem", "BodyText", fontSize=10, leading=14,
...
def _render_inline_children(children) -> str:
    ...
    for tok in children or []:
        ...
        if ttype == "text":
            ...
            out.append("<b>"); bold_depth += 1
        elif ttype == "strong_close":
            if bold_depth > 0:
                out.append("</b>"); bold_depth -= 1
        elif ttype == "em_open":
            out.append("<i>"); italic_depth += 1
        elif ttype == "em_close":
            if italic_depth > 0:
                out.append("</i>"); italic_depth -= 1
        elif ttype == "s_open":
            out.append("<strike>"); strike_depth += 1
        elif ttype == "s_close":
            if strike_depth > 0:
                out.append("</strike>"); strike_depth -= 1
    ...
    while strike_depth > 0:
        out.append("</strike>"); strike_depth -= 1
    while link_stack:
        out.append("</link>"); link_stack.pop()
    return "".join(out)
...
def _build_table(tokens: list, start: int) -> tuple:
    ...
    style = TableStyle([
        ("BACKGROUND", (0, 0), (-1, header_row_count - 1 if header_row_count else 0),
            colors.HexColor("#eef2ff")),
        ("TEXTCOLOR",  (0, 0), (-1, header_row_count - 1 if header_row_count else 0),
            colors.HexColor("#1e3a8a")),
        ("FONTNAME",   (0, 0), (-1, header_row_count - 1 if header_row_count else 0),
            "Helvetica-Bold"),
            ...
            while i < len(tokens) and depth > 0:
                ...
                if tt == "paragraph_open":
                    ...
                    item_flowables.append(
                        Paragraph(_inline_to_html(inline), _LIST_ITEM)
                    )
...
def write_pdf_from_markdown(markdown_text: str, output_path: Path) -> Path:
    ...
    def _on_page(canvas_, _doc) -> None:
        ...
        canvas_.drawString(0.7 * inch, 0.4 * inch,
                            "CV Optimizer — Resume Recommendation Report")
        canvas_.drawRightString(letter[0] - 0.7 * inch, 0.4 * inch,
                                  f"Page {page_num}")
