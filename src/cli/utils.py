...
def validate_cv_path(cv_path: Path) -> str | None:
    ...
    if cv_path.suffix.lower() not in SUPPORTED_CV_EXTS:
        return (f"unsupported CV format: {cv_path.suffix}. "
...
_ROLE_KEYWORDS: dict[str, list[str]] = {
    "Backend Engineer":  ["backend", "api ", "server-side", " go ", "golang",
                          "python", "rust", "node.js", "spring boot", "microservices"],
    "Frontend Engineer": ["frontend", "react", "vue", "angular", "ui/ux", "css",
                          "typescript front", " ux "],
    "Data Scientist":    ["data scientist", "ml engineer", "machine learning",
                          "data science", "modelo", "modelos predictivos"],
    "Project Manager":   ["project manager", "program manager", " pm ",
                          "project management", "administrar proyectos",
                          "gestión de proyectos"],
    "DevOps Engineer":   ["devops", "sre", "platform engineer", "kubernetes",
    ...
    "Marketing Manager": ["marketing manager", "growth marketing",
                          "marketing director", "brand manager", "cmo "],
    "Incident Manager":  ["incident manager", "major incident",
                          "incident response", "mim "],
}
    ...
    try:
        if path.suffix.lower() in (".md", ".txt") and path.exists():
            ...
            try:
                ...
                text += "\n" + " ".join(
                    (pg.extract_text() or "") for pg in pages
                )[:4000].lower()
            except Exception:
                pass
    except Exception:
...
def classify_role_type(name_or_path: Any) -> str:
    ...
    """
    text = (_extract_text_from_path(name_or_path)
            if isinstance(name_or_path, Path)
            else str(name_or_path).lower())

    scores = {label: sum(1 for t in terms if t in text)
              for label, terms in _ROLE_KEYWORDS.items()}
    best_label, best_score = max(scores.items(), key=lambda kv: kv[1],
                                   default=("Practitioner", 0))
