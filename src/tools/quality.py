...
_AI_BUZZWORDS = {
    "results-driven", "results-oriented", "dynamic professional",
    "passionate", "passionate team player", "passionate about",
    "proven track record", "synergy", "synergies", "leverage",
    "leveraging", "spearheaded", "innovative solutions", "team mindset",
    "highly motivated", "detail-oriented", "go-getter", "self-starter",
    "best-in-class", "world-class", "cutting-edge", "thought leader",
    "value-add", "deliverables", "stakeholders", "actionable insights",
    "transformative", "robust solutions", "seamless integration",
    "ecosystem", "paradigm shift", "game-changer", "mission-critical",
...
def detect_ai_smell(text_json: str) -> str:
    ...
    for m in re.finditer(r"(\b\w+(?:\s+\w+){0,3})\s*\b\d{1,3}(?:\.\d+)?%", text):
        ...
        if not any(w in prefix for w in
                   ["by", "through", "via", "using", "after", "from", "with",
                    "improved", "reduced", "increased"]):
            metric_credibility_issues.append(m.group(0))

    score = (min(len(detected_buzzwords) * 8, 50)
    ...
    if detected_buzzwords:
        suggested.append(f"Replace generic buzzwords ({', '.join(detected_buzzwords[:5])}) "
...
def self_critique_bullet(input_json: str) -> str:
    ...
    rew_numbers = set(re.findall(r"\b\d{1,3}(?:\.\d+)?%|\b\d+\s*(?:engineers|services|teams|years)",
                                  rewritten))
    orig_numbers = set(re.findall(r"\b\d{1,3}(?:\.\d+)?%|\b\d+\s*(?:engineers|services|teams|years)",
                                   original))
    for n in rew_numbers - orig_numbers:
        issues.append(f"unsupported number: {n}")

    # Stronger verb in rewritten than original?
    weak_strong = {"led": ["collaborated", "contributed", "helped"],
    ...
    for strong, weaks in weak_strong.items():
        if strong in rewritten and any(w in original for w in weaks):
            if strong not in original:
                issues.append(f"verb escalation: original was '{[w for w in weaks if w in original][0]}', "
