...
def extract_voice_signature(text_json: str) -> str:
    ...
    contractions = sum(
        1 for c in ["i'm", "i've", "i'd", "don't", "can't", "won't"] if c in text_lower
    )
    formal_markers = sum(
        1 for f in ["furthermore", "moreover", "additionally", "consequently"] if f in text_lower
    )
    formality = (
        "casual"
        if contractions > formal_markers
        else "formal"
        if formal_markers > contractions + 2
        else "balanced"
    )

    metric_count = len(
        re.findall(r"\d+(?:\.\d+)?\s*(?:%|x|k|m|b|hours?|days?|weeks?|months?|years?)", text_lower)
    )
