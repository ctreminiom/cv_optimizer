...
def test_job_posting_completeness_accepts_valid():
    ok, _ = job_posting_completeness(json.dumps({
...
def test_job_posting_completeness_rejects_empty_title():
    ok, msg = job_posting_completeness(json.dumps({
        "title": "",
        "requirements": [1, 2, 3],
    }))
    assert ok is False
...
def test_candidate_profile_completeness_accepts_valid():
    ok, _ = candidate_profile_completeness(json.dumps({
...
def test_candidate_profile_completeness_rejects_empty_work():
    ok, msg = candidate_profile_completeness(json.dumps({
        "work_experience": [],
        "skills": ["Go"],
    }))
    assert ok is False
...
def test_consolidated_feedback_present_rejects_empty_changes():
    ok, msg = consolidated_feedback_present(json.dumps({
