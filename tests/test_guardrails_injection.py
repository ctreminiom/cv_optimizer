...
def test_adapted_cv_passes_with_clean_mock() -> None:
    def clean_smell(_):
        return json.dumps({"ai_smell_score": 10, "issues": []})

    def clean_critique(_):
        return json.dumps({"issues": []})

    data = _make_adapted_cv(["Reduced API latency by 40% through caching."])
    ok, _ = adapted_cv_no_fabrication(
        data, smell_detector=clean_smell, self_critique_fn=clean_critique
    )
    assert ok is True
...
def test_adapted_cv_fails_on_high_ai_smell() -> None:
    def smelly(_):
        return json.dumps({"ai_smell_score": 75, "issues": ["buzzword overuse"]})

    def clean_critique(_):
        return json.dumps({"issues": []})

    data = _make_adapted_cv(["Leveraged synergies to spearhead innovative initiatives."])
    ok, msg = adapted_cv_no_fabrication(
        data, smell_detector=smelly, self_critique_fn=clean_critique
    )
    assert ok is False
...
def test_adapted_cv_fails_on_many_critique_flags() -> None:
    def clean_smell(_):
        return json.dumps({"ai_smell_score": 5, "issues": []})

    def flaggy(_):
        return json.dumps({"issues": ["passive phrasing"]})
...
def test_adapted_cv_empty_sections_fails() -> None:
    def clean_smell(_):
        return json.dumps({"ai_smell_score": 0, "issues": []})
