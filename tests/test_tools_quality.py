...
def test_detect_ai_smell_clean_text() -> None:
    payload = json.dumps({"text": "Built a distributed caching layer that cut p99 latency by 40ms."})
...
def test_detect_ai_smell_buzzword_heavy() -> None:
    payload = json.dumps({"text": "Leveraged synergies to spearhead innovative solutions and facilitate cross-functional collaboration."})
    result = json.loads(detect_ai_smell.run(payload))
